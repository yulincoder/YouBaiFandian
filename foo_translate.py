...
def print_db(db):           # 打印历史记录详细信息
    ...
    for each in sorted_count:                   # 根据排序输出查询历史记录
        ...
        for item in tmp_for_sort[each]:         # 统计次数一样的不同单词都在list里面
            ...
            print '... ',
            print db.findvalue(item, 'interpert')   # 输出解释
...
if __name__ == '__main__':
	...
	db = mousedb.load('/home/tete/software/YouBaiFandian-master/YouBai.db', True) # 连接数据库，数据库不存在会自己建立
...
只能查询从英语 -> 汉语，不能 从汉语 -> 英语\n\n\
	...
	topbar_msg = 'YouBai [0.0.0.1] \nType ":q", ":c", ":h" or ":his" for more information.'
