...
def get_result_word(word):
    content = url_open(
        'http://dict.youdao.com/w/eng/' + word + '/#keyfrom=dict2.index')
    content = content.replace(' ', '')  #
    content = content.replace('\n', '')  #
    content = content.replace('\t', '')  #
    content = content.replace('\t', '')  #
    result_string = re.findall('"trans-container"><ul><li>(.*?)</li></ul>',
                               content)
    ''' 音标功能有问题，日后再加S
...
def print_db(db):  # 打印历史记录详细信息
    ...
    for each in sorted_count:  # 根据排序输出查询历史记录
        for item in tmp_for_sort[each]:  # 统计次数一样的不同单词都在list里面
            ...
            print '\t[' + str(db.findvalue(item,
                                           'Counter')) + ']'  # 输出单词历史查询次数
            clr_control.set_print_yellow_text()  # 黄色输出
            print '...  ',
            print db.findvalue(item, 'interpert').decode('utf-8').encode(
                'gbk')  # 输出解释
...
def print_summary_db(db, amount_word_a_row=3):  # 打印历史记录简单信息
    ...
    for item in items:  # 按照历史查询频率对单词分类
        ...
        if index not in tmp_for_sort:
            tmp_for_sort[index] = [
                item
            ]  # 将字典的键设置为查询次数，值设置为单词本身，这样可以用sorted函数根据历史查询次数对单词排序
        else:
            tmp_for_sort[index].append(item)
    ...
    for each in sorted_count:  # 根据排序输出查询历史记录
        for word in tmp_for_sort[each]:  # 统计次数一样的不同单词都在list里面
            ...
            clr_control.set_print_red_text()
            print '%6s' % (
                '[' + str(db.findvalue(word, 'Counter')) + ']'),  # 历史查询频率显示
            clr_control.set_print_yellow_text(
            ) if row_cnt % 2 == 0 else clr_control.set_print_green_text()
...
if __name__ == '__main__':
    ...
    topbar_msg = 'YouBai [0.0.0.1] \nType ":q", ":c", ":h" or ":his" for more information.'.decode(
        'utf-8').encode('gbk')
